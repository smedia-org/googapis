...
pub mod storage_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        #[doc = " The `resource` field in the request should be"]
        #[doc = " `projects/_/buckets/{bucket}`."]
        ...
        #[doc = " Tests a set of permissions on the given bucket or object to see which, if"]
        #[doc = " any, are held by the caller."]
        #[doc = " The `resource` field in the request should be"]
        #[doc = " `projects/_/buckets/{bucket}` for a bucket or"]
        #[doc = " `projects/_/buckets/{bucket}/objects/{object}` for an object."]
        ...
        #[doc = " Permanently deletes a NotificationConfig."]
        ...
        #[doc = " Creates a NotificationConfig for a given bucket."]
        #[doc = " These NotificationConfigs, when triggered, publish messages to the"]
        #[doc = " specified Pub/Sub topics. See"]
        #[doc = " https://cloud.google.com/storage/docs/pubsub-notifications."]
        ...
        #[doc = " Retrieves a list of NotificationConfigs for a given bucket."]
        ...
        #[doc = " Deletes an object and its metadata."]
        #[doc = ""]
        #[doc = " Deletions are normally permanent when versioning is disabled or whenever"]
        #[doc = " the generation parameter is used. However, if soft delete is enabled for"]
        #[doc = " the bucket, deleted objects can be restored using RestoreObject until the"]
        #[doc = " soft delete retention period has passed."]
        ...
        #[doc = " Restores a soft-deleted object."]
        ...
        #[doc = " Cancels an in-progress resumable upload."]
        #[doc = ""]
        #[doc = " Any attempts to write to the resumable upload after cancelling the upload"]
        #[doc = " will fail."]
        #[doc = ""]
        #[doc = " The behavior for currently in progress write operations is not guaranteed -"]
        #[doc = " they could either complete before the cancellation or fail if the"]
        #[doc = " cancellation completes first."]
        ...
        #[doc = " `StartResumableWrite()`, populating a `WriteObjectSpec` into that request."]
        #[doc = " They should then attach the returned `upload_id` to the first message of"]
        #[doc = " each following call to `WriteObject`. If the stream is closed before"]
        #[doc = " finishing the upload (either explicitly by the client or due to a network"]
        #[doc = " error or an error response from the server), the client should do as"]
        #[doc = " follows:"]
        #[doc = "   - Check the result Status of the stream, to determine if writing can be"]
        #[doc = "     resumed on this stream or must be restarted from scratch (by calling"]
        #[doc = "     `StartResumableWrite()`). The resumable errors are DEADLINE_EXCEEDED,"]
        #[doc = "     INTERNAL, and UNAVAILABLE. For each case, the client should use binary"]
        #[doc = "     exponential backoff before retrying.  Additionally, writes can be"]
        #[doc = "     resumed after RESOURCE_EXHAUSTED errors, but only after taking"]
        #[doc = "     appropriate measures, which may include reducing aggregate send rate"]
        #[doc = "     across clients and/or requesting a quota increase for your project."]
        #[doc = "   - If the call to `WriteObject` returns `ABORTED`, that indicates"]
        #[doc = "     concurrent attempts to update the resumable write, caused either by"]
        #[doc = "     multiple racing clients or by a single client where the previous"]
        #[doc = "     request was timed out on the client side but nonetheless reached the"]
        #[doc = "     server. In this case the client should take steps to prevent further"]
        #[doc = "     concurrent writes (e.g., increase the timeouts, stop using more than"]
        #[doc = "     one process to perform the upload, etc.), and then should follow the"]
        #[doc = "     steps below for resuming the upload."]
        #[doc = "   - For resumable errors, the client should call `QueryWriteStatus()` and"]
        #[doc = "     then continue writing from the returned `persisted_size`. This may be"]
        #[doc = "     less than the amount of data the client previously sent. Note also that"]
        #[doc = "     it is acceptable to send data starting at an offset earlier than the"]
        #[doc = "     returned `persisted_size`; in this case, the service will skip data at"]
        #[doc = "     offsets that were already persisted (without checking that it matches"]
        #[doc = "     the previously written data), and write only the data starting from the"]
        #[doc = "     persisted offset. Even though the data isn't written, it may still"]
        #[doc = "     incur a performance cost over resuming at the correct write offset."]
        #[doc = "     This behavior can make client-side handling simpler in some cases."]
        #[doc = "   - Clients must only send data that is a multiple of 256 KiB per message,"]
        #[doc = "     unless the object is being finished with `finish_write` set to `true`."]
        #[doc = ""]
        ...
        #[doc = ""]
        #[doc = " Attempting to resume an already finalized object will result in an OK"]
        #[doc = " status, with a WriteObjectResponse containing the finalized object's"]
        #[doc = " metadata."]
        #[doc = ""]
        #[doc = " Alternatively, the BidiWriteObject operation may be used to write an"]
        #[doc = " object with controls over flushing and the ability to fetch the ability to"]
        #[doc = " determine the current persisted size."]
        ...
        #[doc = " Stores a new object and metadata."]
        #[doc = ""]
        #[doc = " This is similar to the WriteObject call with the added support for"]
        #[doc = " manual flushing of persisted state, and the ability to determine current"]
        #[doc = " persisted size without closing the stream."]
        #[doc = ""]
        #[doc = " The client may specify one or both of the `state_lookup` and `flush` fields"]
        #[doc = " in each BidiWriteObjectRequest. If `flush` is specified, the data written"]
        #[doc = " so far will be persisted to storage. If `state_lookup` is specified, the"]
        #[doc = " service will respond with a BidiWriteObjectResponse that contains the"]
        #[doc = " persisted size. If both `flush` and `state_lookup` are specified, the flush"]
        #[doc = " will always occur before a `state_lookup`, so that both may be set in the"]
        #[doc = " same request and the returned state will be the state of the object"]
        #[doc = " post-flush. When the stream is closed, a BidiWriteObjectResponse will"]
        #[doc = " always be sent to the client, regardless of the value of `state_lookup`."]
